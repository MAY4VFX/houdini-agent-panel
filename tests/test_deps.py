...
def test_find_hython_without_a_version_uses_hfs(tmp_path, monkeypatch):
    """`--houdini-dir /studio/hsite/packages` has no version in its name, so
    `houdini_version_of` returns None for it. With `$HFS` set — which it
    always is under `hython`, the documented way to run the installer —
    that used to reach `re.escape(None)` and end the install with a
    TypeError."""
