...
def test_all_row_labels_share_one_left_edge(qapp):
    ...
    assert None not in (default_agent_label, whisper_label, data_folder_label)

    xs = {_left_x(view, w) for w in (default_agent_label, whisper_label, data_folder_label)}
    assert len(xs) == 1, f"labels do not share one X: {xs}"


def test_all_row_values_share_one_left_edge(qapp):
    """The combo, the whisper edit and the data-folder row all start at the
    same X: the label column's fixed width plus the gap — not wherever
    each section's own longest label happened to end."""
    view = _build(qapp)
