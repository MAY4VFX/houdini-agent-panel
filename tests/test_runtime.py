...
# --- config isolation: launch_spec / install_agent ---------------------------
#
# `Settings.isolate_agent_config` — see runtime.py::_with_config_isolation.
# Only `claude-acp` has a verified variable (`CLAUDE_CONFIG_DIR`, read
# straight from `claude-agent-acp`'s own bundled source); every other agent
# id must be left alone rather than guess at a mechanism.


def _claude_npx_entry(version: str = "1.0.0") -> AgentEntry:
    return AgentEntry(
        id="claude-acp",
        name="Claude Agent",
        version=version,
        npx=NpxDistribution(package="@agentclientprotocol/claude-agent-acp", args=[], env={}),
    )


def test_isolation_off_by_default_leaves_config_dir_unset(monkeypatch):
    monkeypatch.setattr("houdini_agent_panel.node.ensure_node", lambda **k: Path("/fake/node"))
    monkeypatch.setattr(
        "houdini_agent_panel.node.npx_argv",
        lambda node_bin, package, args: [str(node_bin), "/fake/npx-cli.js", "--yes", package, *args],
    )
    entry = _claude_npx_entry()
    runtime.install_agent(entry, settings=Settings())
...
def test_isolation_on_points_claude_at_a_panel_owned_directory(monkeypatch):
    monkeypatch.setattr("houdini_agent_panel.node.ensure_node", lambda **k: Path("/fake/node"))
    monkeypatch.setattr(
        "houdini_agent_panel.node.npx_argv",
        lambda node_bin, package, args: [str(node_bin), "/fake/npx-cli.js", "--yes", package, *args],
    ...
    spec = runtime.launch_spec(entry, settings=Settings(isolate_agent_config=True))

    assert spec.env["CLAUDE_CONFIG_DIR"] == str(paths.agent_config_dir("claude-acp"))


def test_isolation_is_a_no_op_for_an_agent_with_no_verified_variable(monkeypatch):
    """The design rule this exists to honor: an agent id we have not
    verified must never get an invented environment variable."""
    monkeypatch.setattr("houdini_agent_panel.node.ensure_node", lambda **k: Path("/fake/node"))
    monkeypatch.setattr(
        "houdini_agent_panel.node.npx_argv",
        lambda node_bin, package, args: [str(node_bin), "/fake/npx-cli.js", "--yes", package, *args],
    )
    entry = _npx_entry()  # id="test-npx-agent" — not in _CONFIG_DIR_ENV_VAR
    runtime.install_agent(entry, settings=Settings())
...
def test_isolation_never_overrides_an_explicit_config_dir_from_the_agents_own_env(monkeypatch):
    """An artist (or a Custom Agent's own `env`) who already set
    `CLAUDE_CONFIG_DIR` made a deliberate choice — the panel's own default
    for the setting must not clobber it."""
    monkeypatch.setattr("houdini_agent_panel.node.ensure_node", lambda **k: Path("/fake/node"))
    monkeypatch.setattr(
        "houdini_agent_panel.node.npx_argv",
        lambda node_bin, package, args: [str(node_bin), "/fake/npx-cli.js", "--yes", package, *args],
    ...
    entry = AgentEntry(
        ...
        npx=NpxDistribution(
            ...
            args=[],
            env={"CLAUDE_CONFIG_DIR": "/artist/chosen/dir"},
        ),
    )
    runtime.install_agent(entry, settings=Settings())
    ...
    assert spec.env["CLAUDE_CONFIG_DIR"] == "/artist/chosen/dir"


def test_isolation_does_not_touch_a_captured_oauth_token(monkeypatch):
    """Answers the question this setting raises on its own: isolating the
    config directory must not also isolate away the sign-in this panel
    already captured — `CLAUDE_CODE_OAUTH_TOKEN` is read from the
    environment, never from a file under `CLAUDE_CONFIG_DIR` (verified in
    the Claude Agent SDK's own bundle)."""
    monkeypatch.setattr("houdini_agent_panel.node.ensure_node", lambda **k: Path("/fake/node"))
    monkeypatch.setattr(
        "houdini_agent_panel.node.npx_argv",
        lambda node_bin, package, args: [str(node_bin), "/fake/npx-cli.js", "--yes", package, *args],
