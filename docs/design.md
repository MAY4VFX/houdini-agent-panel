...
- **Houdini's macOS bundle can't record audio, and it isn't a bug we can fix.** None of the ten installed bundles checked (20.5/21.0/22.0, every edition) declare `NSMicrophoneUsageDescription` in `Info.plist`. macOS's TCC never shows Houdini in Privacy & Security for that reason, and Qt6 (`QMicrophonePermission`, H22 only — Qt5/PySide2 on 20.5 has no such API) refuses to even request access without that key: `qt.permissions: Requesting QMicrophonePermission requires "NSMicrophoneUsageDescription" in Info.plist`. Recording still "succeeds" — no error, no crash — and produces a file with the WAV header and zero audio samples.
- **Voice input is off everywhere, unconditionally, until it's verified per platform.** macOS is the only platform actually measured (the fact above); Linux and Windows have never been tried, so an earlier fix that showed voice input there anyway — on the unverified assumption that it'd be fine — was a guess standing in for a measurement, not a real per-platform check. `ui/voice.py::_VOICE_INPUT_AVAILABLE` is a single unconditional kill switch, read by both `SettingsView` (hides the Voice section) and `VoiceButton.configure` (hides the mic button); see that flag's own comment for what flipping it back requires.
...
**Panel settings** (a minimal set, its own screen): default agent; auto-start the agent when the panel opens; check for updates; show announcements; telemetry; the data folder with an "Open" button; a local whisper endpoint for voice on agents lacking the `audio` capability, currently hidden unconditionally pending per-platform verification (see the voice-input fact above); "Copy diagnostics" for bug reports.
...
**Bottom** — a growing input field. On the left: a "+" for files (only with `image`/`embeddedContext`), a microphone (only with `audio` or a configured local whisper, and currently never — voice input is off unconditionally pending per-platform verification, see the fact above), a mode chip built from `availableModes`. On the right — a counter from `usage_update` and a send button that turns into a stop button.
